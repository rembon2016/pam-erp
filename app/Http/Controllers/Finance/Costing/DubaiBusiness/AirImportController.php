...
final class AirImportController extends Controller
{
    ...
    public function cost($id)
    {
        ...
        if ($cost->exists()) {
            ...
            $data = [
                'action' => route('finance.costing.dubai-business.air-import.update', $costing->id),
                'method' => 'PUT',
            ];
        } else {
            $costing = null;
            $data = [
                'action' => route('finance.costing.dubai-business.air-import.store'),
                'method' => 'POST',
            ];
        }

        return view('pages.finance.costing.dubai-business.air-import.cost', compact('joborder', 'vendor_all', 'vendor_truck', 'vendor_port', 'vendor_air', 'vendor_line', 'charge', 'currency', 'loadingplan', 'costing', 'data', 'loading'));
    ...
    public function store(Request $request)
    {
        ...
        foreach ($loadingplan as $j => $rx) {
            if (! empty($request["vendor_mawb_{$j}_id"])) {
                $costing_head = $this->costingService->createCostingHead($request, $costing->id, $rx->mawb_number, 'mawb', 'AIRIMPORT', $rx->plan_id);

                $this->costingService->createCostingDetailMawb($request, $costing->id, $rx->mawb_number, $shipment_by, $costing_head->id, $j);
            }
        }

        return to_route('finance.costing.dubai-business.air-import.index')->with('toastSuccess', 'Costing Success');
    }
    ...
    public function update($id, Request $request)
    {
        ...
        foreach ($loadingplan as $j => $rx) {
            if (! empty($request["vendor_mawb_{$j}_id"])) {
                ...
                $costing_head = $this->costingService->updateCostingHead($request, $id, $rx->mawb_number, 'mawb', 'AIRIMPORT', $rx->plan_id);
        ...
        return to_route('finance.costing.dubai-business.air-import.index')->with('toastSuccess', 'Update Success');
    }

    public function contractmawb($id, $mawb_number, $type)
    {
        $result = $this->calculationService->getChargeByMawb($id, $mawb_number, $type, 'AIRIMPORT');
    ...
    {
        $result = $this->calculationService->getChargeExportByLp($id, $loading_id, 'AIRIMPORT');
    ...
    public function status($id, $status)
    {
        ...
        return to_route('finance.costing.dubai-business.air-import.show', $costing->job_order_id)->with('toastSuccess', 'Update Success');
    }
