...
use App\Exports\Costing\DubaiBusiness\SeaImportExport;
...
final class SeaImportController extends Controller
{
    ...
    {
        return view('pages.finance.costing.dubai-business.sea-import.index');
    }

    public function list(Request $request): JsonResponse
    {
        if (request()->ajax()) {
            $count_total = JobOrder::select('job_order_id')->where('job_order_type', 'SEAIMPORT')->where('status', '!=', 3)->count();
            $start = ($request->start) ? $request->start : 0;
            $pageSize = ($request->length) ? $request->length : 10;
            $draw = $request->query('draw');

            $joborder = JobOrder::with(['detail', 'loading','origin','costing'])->where('job_order_type', 'SEAIMPORT')->where('status', '!=', 3);
    ...
    public function show($id)
    {
        ...
        return view('pages.finance.costing.dubai-business.sea-import.show', compact('id', 'joborder', 'op', 'lpdoc', 'costing'));
    }
    ...
    public function cost($id) {
        ...
        if ($cost->exists()) {
            ...
            $data = [
                'action' => route('finance.costing.dubai-business.sea-import.update', $costing->id),
                'method' => 'PUT',
            ];
        } else {
            $costing = null;
            $details = null;
            $data = [
                'action' => route('finance.costing.dubai-business.sea-import.store'),
                'method' => 'POST',
            ];
        }

        return view('pages.finance.costing.dubai-business.sea-import.cost', compact('id', 'joborder', 'loading', 'vendor_truck', 'vendor_port', 'vendor_air', 'vendor_line', 'charge', 'currency', 'bl','costing', 'data', 'vendor_all', 'details'));
    }
    ...
    public function store(Request $request)
    {
        ...
        foreach ($bl as $j => $rx) {
            if (! empty($request["vendor_bl_{$j}_id"])) {
                $costing_head = $this->costingService->createCostingHead($request, $costing->id, $rx->bl_number, 'bl', 'SEAIMPORT', $rx->loading_report_bl_id);
        ...
        return to_route('finance.costing.dubai-business.sea-import.index')->with('toastSuccess', 'Costing Success');
    }
    ...
    public function update($id, Request $request)
    {
        ...
        foreach ($bl as $j => $rx) {
            if (! empty($request["vendor_bl_{$j}_id"])) {
                ...
                $costing_head = $this->costingService->updateCostingHead($request, $id, $rx->bl_number, 'bl', 'SEAIMPROT', $rx->loading_report_bl_id);
        ...
        return to_route('finance.costing.dubai-business.sea-import.index')->with('toastSuccess', 'Update Success');
    }
    ...
    public function status($id, $status)
    {
        ...
        return to_route('finance.costing.dubai-business.sea-import.show', $costing->job_order_id)->with('toastSuccess', 'Update Success');
    }

    public function exportCsv()
    {
        $file_name = 'list_costing_sea_import_'.time().'.csv';

        return Excel::download(new SeaImportExport, $file_name);
    }
}
