...
use App\Exports\Costing\DubaiBusiness\TruckingExport;
...
final class TruckingController extends Controller
{
    ...
    {
        return view('pages.finance.costing.dubai-business.trucking.index');
    }

    public function list(Request $request): JsonResponse
    {
        if (request()->ajax()) {
            $count_total = JobOrder::select('job_order_id')->where('job_order_type', 'TRUCKING')->where('status', '!=', 3)->count();
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($item) {
                    return Utility::generateTableActions([
                        'show' => route('finance.costing.dubai-business.trucking.show', $item->job_order_id),
                        'cost' => route('finance.costing.dubai-business.trucking.cost', $item->job_order_id),
                    ]);
                })
                ->rawColumns(['action'])
    ...
    public function show($id)
    {
        ...
        return view('pages.finance.costing.dubai-business.trucking.show', compact('id', 'joborder', 'op', 'lpdoc', 'costing'));
    }
    ...
    public function cost($id) {
        ...
        if ($cost->exists()) {
            ...
            $data = [
                'action' => route('finance.costing.dubai-business.trucking.update', $costing->id),
                'method' => 'PUT',
            ];
        } else {
            $costing = null;
            $details = null;
            $data = [
                'action' => route('finance.costing.dubai-business.trucking.store'),
                'method' => 'POST',
            ];
        }

        return view('pages.finance.costing.dubai-business.trucking.cost', compact('id', 'joborder','vendor_truck', 'vendor_port', 'vendor_air', 'vendor_line', 'charge', 'currency','costing', 'data', 'vendor_all', 'details'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $this->costingService->createCostingSpecialOther($request, $costing->id);
            ...
            if (! empty($request["vendor_other_0_id"])) {
                $costing_head = $this->costingService->createCostingHead($request, $costing->id, $joborder->loading_plan_number, 'other', 'TRUCKING', $joborder->loading_plan_id);
        ...
        return to_route('finance.costing.dubai-business.trucking.index')->with('toastSuccess', 'Costing Success');
    }
    ...
    public function update($id, Request $request)
    {
        ...
        $this->costingService->updateCostingSpecialOther($request, $id);
            ...
            if (! empty($request["vendor_other_0_id"])) {
                ...
                $costing_head = $this->costingService->updateCostingHead($request, $id, $joborder->loading_plan_number, 'other', 'TRUCKING', $joborder->loading_plan_id);
        ...
        return to_route('finance.costing.dubai-business.trucking.index')->with('toastSuccess', 'Update Success');
    }
    ...
    {
        $file_name = 'list_costing_trucking_'.time().'.csv';

        return Excel::download(new TruckingExport, $file_name);
    }
}
