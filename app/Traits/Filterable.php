			...
			if($filter = $this->isFilterAvailable($key, $filter_fields)){
				if(substr($i,0,1) != "!"){
					if(substr($i,0,4) == "btw:"){
						...
						}else if($this->validateDate($start, $this->getDateFormatFilter())){
							if($operator == 'or'){
								$query->orWhere(function($query2)use($filter, $start, $end){
									$query2->where($filter, '>=', $start);
					...
					}else if($this->validateDate($i, $this->getDateFormatFilter()) && $this->getDateFormatFilter() === 'Y-m-d'){
						if($operator == 'or'){
					...
					}else if(in_array(substr($i,1,2), ['<=', '>='])){
						$query->whereNot($filter, substr($i,1,2), substr($i,3));
					}else if(in_array(substr($i,1,1), ['<', '>'])){
						$query->whereNot($filter, substr($i,1,1), substr($i,2));
					}else if(substr($i,1) == 'null' || substr($i,1) === null){
