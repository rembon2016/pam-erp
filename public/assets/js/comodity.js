const getInitialData = async (filter = '', filter_value = '') => {
    try {
      ...
      const res = await axios.get(
        `${user}api/dashboard/si-overview/comodity?role_id=99&field=&limit=5&filter=${filter}&filter_value=${filter_value}`
      );
  ...
  const getListData = async (filter = '', filter_value = '',download = '') => {
    try {
     
      const res = await axios.get(
        `${user}api/dashboard/si-overview/comodity?role_id=99&field=&limit=&page=&filter=${filter}&filter_value=${filter_value}&download=${download}`
      );
      const data = res?.data;
      return data;
    } catch (error) {
...
getListData()
  .then(data => {
    ...
    chartAllData(data.data,"commodity");
  })
  .catch(error => {
...
function chartBar(data){
  var datas = data;
   const optionsBar = {
    chart: {
      events: {
        click: function (e) {
          ...
          window.location.href="/pds/chart/country?mode=comodity&filter=commodity&filter_value="+data[categoryIndex]?.id;
    ...
    tooltip: {
      headerFormat:
        '<div style="font-size:10px; background: rgba(0,0,0,0.1); padding:4px 10px; display: flex; justify-content: center; border-radius:20px;"><i style="margin-right:5px;" class="fas fa-spa"></i>{point.key}</div><div>',
      pointFormat:
        ...
        '<div><div style="color:{series.color};">Gross-Weight :<span style="color: darkcyan; font-weight: 600;"> {point.gw}</span></div>' +
    ...
    series: [
      {
        point: {
          events: {
            click: function (e) {
              ...
              window.location.href="/pds/chart/country?mode=comodity&filter=commodity&filter_value="+data[index]?.id;
              
            },
          },
        },
        name: `Data Chart Commodity`,
        color: {
          linearGradient: {
            x1: 0,
            x2: 0,
        ...
        keys: ["y", "cw", "cbm", "gw", "teus", "ctd", "color"],
        dataLabels: {
          enabled: false, // show data diatas grafik
          color: "#000000",
          align: "right",
    ...
    events: {
      ...
      Highcharts.chart('chart-comodity', optionsBar);
...
$( "#filter" ).on( "click", function() {
    ...
    $('#chart-comodity').highcharts().destroy();
...
$( "#download" ).on( "click", async function() {
    try {
        ...
        const apiUrl = `${user}api/dashboard/si-overview/comodity?role_id=99&field=&limit=&page=&filter=${filterParams.filter}&filter_value=${filterParams.filter_value}&download=true`;

        await downloadFileExcel(apiUrl, "comodity");
