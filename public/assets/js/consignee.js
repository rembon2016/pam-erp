const getInitialData = async (filter = '', filter_value = '') => {
    try {
      ...
      const res = await axios.get(
        `${user}api/dashboard/si-overview/consignee?role_id=99&field=&limit=5&filter=${filter}&filter_value=${filter_value}`
      );
  ...
  const getListData = async (filter = '', filter_value = '') => {
    try {
      ...
      const res = await axios.get(
        `${user}api/dashboard/si-overview/consignee?role_id=99&field=&limit=&page=&filter=${filter}&filter_value=${filter_value}`
      );
      const data = res?.data;
      return data;
    } catch (error) {
...
getListData()
  .then(data => {
    ...
    chartAllData(data.data,"consignee");
  })
  .catch(error => {
...
function chartBar(data){
  var datas = data;
    ...
    chart: {
      events: {
        click: function (e) {
          ...
          window.location.href="/pds/chart/shipment?mode=consignee&id="+data[categoryIndex]?.id;
    ...
    tooltip: {
      headerFormat:
        '<div style="font-size:10px; background: rgba(0,0,0,0.1); padding:4px 10px; display: flex; justify-content: center; border-radius:20px;"><i style="margin-right:5px;" class="fas fa-dolly-flatbed"></i>{point.key}</div><div>',
      pointFormat:
        ...
        '<div style="margin-top:2px;"><div style="color:{series.color}; padding: 0">Country :<span style="color: darkcyan; font-weight: 600;"> {point.country}</span></div>' +
          ...
          events: {
            click: function (e) {
              ...
              const data = datas?.map((x, i) => {
                return {
                  id: x?.customer_id,
                  name: x?.customer_name ?? 0,
                  index: i,
                };
              });

              window.location.href="/pds/chart/shipment?mode=consignee&id="+data[index]?.id;
            
            },
          },
        },
        name: `Data Chart Consignee`,
        color: {
          linearGradient: {
            x1: 0,
            x2: 0,
    ...
    events: {
      ...
      Highcharts.chart('chart-consignee', optionsBar);
    ...
    data.map((data) => {
        html += `
            <tr>
                ...
                <td>${data?.country_origin_consignee}</td>
...
$( "#filter" ).on( "click", function() {
    ...
    var group_consignee = $("#group_id").val();
    ...
    var filters = {
        ...
        group_consignee: group_consignee
    };

    var filterParams = buildFilterParams2(filters);
    console.log(filterParams);
    $('#chart-consignee').highcharts().destroy();
...
$( "#download" ).on( "click", async function() {
  ...
  var filterParams = buildFilterParams2(filters);
  const apiUrl = `${user}api/dashboard/si-overview/consignee?role_id=99&field=&limit=&page=&filter=${filterParams.filter}&filter_value=${filterParams.filter_value}&download=true`;

  await downloadFileExcel(apiUrl, "consignee");
