const getInitialData = async (filter = '', filter_value = '') => {
    try {
      ...
      const res = await axios.get(
        `${user}api/dashboard/si-overview/origin?role_id=99&field=&limit=5&filter=${filter}&filter_value=${filter_value}`
      );
  ...
  const getListData = async (filter = '', filter_value = '') => {
    try {
      ...
      const res = await axios.get(
        `${user}api/dashboard/si-overview/origin?role_id=99&field=&limit=&page=&filter=${filter}&filter_value=${filter_value}`
      );
      const data = res?.data;
      return data;
    } catch (error) {
...
function chartBar(data){
  var datas = data;
    ...
    chart: {
      events: {
        click: function (e) {
          ...
          window.location.href="/pds/chart/detail?mode=origin&id="+data[categoryIndex]?.id+"&name="+data[categoryIndex]?.name_office;
    ...
    tooltip: {
      headerFormat:
        '<div style="font-size:10px; background: rgba(0,0,0,0.1); padding:4px 10px; display: flex; justify-content: center; border-radius:20px;"><i style="margin-right:5px;" class="fas fa-map-marker-alt"></i>{point.key}</div><div>',
      pointFormat:
        ...
        '<div><div style="color:{series.color};">Country :<span style="color: darkcyan; font-weight: 600;"> {point.country}</span></div>' +
        '<div style="font-size:11px; font-weight:400; margin-top:4px">(Click to see detail)</div>',
      footerFormat: "</div>",
      shared: true,
      useHTML: true,
    },
    series: [
      {
          ...
          events: {
            click: function (e) {
              ...
              const data = datas?.map((x, i) => {
                return {
                  id: x?.office_id,
                  name_office: x?.office_name,
                  index: i,
                };
              });
              window.location.href="/pds/chart/detail?mode=origin&id="+data[index]?.id+"&name="+data[index]?.name_office;
              
            },
          },
        },
        name: `Data Chart Origin`,
        // ${
        //   props.dateStart === "" || props.dateEnd === ""
        //     ? ""
        //     : `from ${new Date(props.dateStart).toLocaleDateString("en-en", {
        //         // weekday: "long",
        //         year: "numeric",
        //         month: "long",
        //         day: "numeric",
        //       })} to ${new Date(props.dateEnd).toLocaleDateString("en-en", {
        //         // weekday: "long",
        //         year: "numeric",
        //         month: "long",
        //         day: "numeric",
        ...
        data: data?.map((data) => {
          return [
            ...
            data?.teus,
            data?.ctd,
            textCapitalize(data?.countries?.country_name),
          ];
        }),
        keys: [
          "y",
    ...
    events: {
      ...
      Highcharts.chart('chart-origin', optionsBar);
...
$( "#filter" ).on( "click", function() {
    ...
    $('#chart-origin').highcharts().destroy();
...
$( "#download" ).on( "click", async function() {
  ...
  var filterParams = buildFilterParams2(filters);
  const apiUrl = `${user}api/dashboard/si-overview/origin?role_id=99&field=&limit=&page=&filter=${filterParams.filter}&filter_value=${filterParams.filter_value}&download=true`;

  await downloadFileExcel(apiUrl, "origin");
