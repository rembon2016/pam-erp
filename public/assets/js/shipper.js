...
const getInitialData = async (filter = '', filter_value = '') => {
    try {
      ...
      const res = await axios.get(
        `${user}api/dashboard/si-overview/shipper?role_id=99&field=&limit=5&filter=${filter}&filter_value=${filter_value}`
      );
  ...
  const getListData = async (filter = '', filter_value = '') => {
    try {
      ...
      const res = await axios.get(
        `${user}api/dashboard/si-overview/shipper?role_id=99&field=&limit=&page=&filter=${filter}&filter_value=${filter_value}`
      );
      const data = res?.data;
      return data;
    } catch (error) {
...
getListData()
  .then(data => {
    ...
    chartAllData(data.data,"shipper");
  })
  .catch(error => {
...
function chartBar(data){
  var datas = data;
    ...
    chart: {
      events: {
        click: function (e) {
          ...
          window.location.href="/pds/chart/shipment?mode=shipper&id="+data[categoryIndex]?.id;
         /* if (categoryIndex !== undefined) {
            router.push({
              pathname:
                roleName === "Executive"
                  ? `/executive/shipper/list/${data[categoryIndex]?.id}`
                  : roleName === "Sales"
                  ? `/sales/shipper/list/${data[categoryIndex]?.id}`
                  : `/dxb/chartshipper/list/${data[categoryIndex]?.id}`,
              query: {
                name: data[categoryIndex]?.name,
              },
            });
          }*/
        },
    ...
    tooltip: {
      headerFormat:
        '<div style="font-size:10px; background: rgba(0,0,0,0.1); padding:4px 10px; display: flex; justify-content: center; border-radius:20px;"><i style="margin-right:5px;" class="fas fa-truck"></i>{point.key}</div><div>',
      pointFormat:
        ...
        '<div><div style="color:{series.color};">CTD :<span style="color: darkcyan; font-weight: 600;"> {point.ctd}</span></div>' +
    ...
    series: [
      {
        ...
        name: `Data Chargeable weight by Shipper`, // title dibawah chart
        color: {
          linearGradient: {
            x1: 0,
            x2: 0,
        ...
        keys: ["y", "cw", "cbm", "gw", "teus", "ctd", "country"],
        dataLabels: {
          enabled: false, // show data diatas grafik
          color: "#000000",
          align: "right",
    ...
    events: {
      ...
      Highcharts.chart('chart-shipper', optionsBar);
...
$( "#filter" ).on( "click", function() {
    ...
    $('#chart-shipper').highcharts().destroy();
...
$( "#download" ).on( "click", async function() {
  ...
  const apiUrl = `${user}api/dashboard/si-overview/shipper?role_id=99&field=&limit=&page=&filter=${filterParams.filter}&filter_value=${filterParams.filter_value}&download=true`;

  await downloadFileExcel(apiUrl, "shipper");
