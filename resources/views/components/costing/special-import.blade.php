...
<div>
    ...
    <div id="special-charges-form" class="d-none">
                        ...
                        @foreach ($costing->special as $key => $row)
                            @if ($row->costing_type === 'import')
                                ...
                                <tr id="row-special-import-{{ $key }}">
                                    <td>
                                        <input type="hidden" name="costing_special_import_id[]"
                                            value="{{ $row->id }}">
                                        @if ($costing->status != 1)
                                            <input type="hidden" name="vendor_special_import_id[]" type="text"
                                                class=" form-control" value="{{ $row->vendor_id }}">
                                            <input type="hidden" name="charge_special_import_id[]" type="text"
                                                class=" form-control" value="{{ $row->charge_id }}">
                                            <input type="hidden" name="currency_special_import_id[]" type="text"
                                                class=" form-control" value="{{ $row->currency_id }}">
                                            <input type="hidden" name="status_special_import[]" type="text"
                                        ...
                                        <select class="form-select vendor-select"
                                            onchange="setVendorSpecialImportName({{ $key }})
                                                "
                                            data-control="select2" id="vendor_special_import_id_{{ $key }}"
                                            name="vendor_special_import_id[]" data-key="{{ $key }}"
        ...
        <td>
            ...
            <input type="text" class="form-control" value="{{ $row->vendor_name }}"
                id="vendor_special_import_name_{{ $key }}" placeholder="Name"
                name="vendor_special_import_name[]" readonly>
            @if ($isDuplicateVendor)
    </div>
    @endif
    </td>
    <td>
        <select class="form-select" data-control="select2" id="charge_special_import_id_0"
            name="charge_special_import_id[]" data-key="{{ $key }}"
            @if ($costing->status != 1) disabled @endif>
            <option>Select</option>
    ...
    <td>
        ...
        <select class="form-select" data-control="select2" id="currency_special_import_id_{{ $key }}"
            name="currency_special_import_id[]" data-key="{{ $key }}"
            @if ($costing->status != 1) disabled @endif>
            <option>Select</option>
    ...
    <td><input type="text" class="form-control" name="rate_special_import[]" value="{{ $row->rate }}"
            placeholder="Type here.." @if ($costing->status != 1) readonly @endif></td>
    <td><input type="text" class="form-control" name="amount_special_import[]" value="{{ $row->amount }}"
            placeholder="Type here.." @if ($costing->status != 1) readonly @endif></td>
    <td><input type="text" class="form-control" value="{{ $row->local_amount }}"
            name="local_amount_special_import[]" placeholder="Type here.."
    ...
    @endforeach
@else
    <tr id="row-special-import-0">
        <td><select class="form-select vendor-select"
                onchange="setVendorSpecialImportName(0)
                                                "
                data-control="select2" id="vendor_special_import_id_0" name="vendor_special_import_id[]"
                data-key="0">
                <option>Select</option>
                @foreach ($vendorLine as $rows)
        ...
        <td><input type="text" class="form-control" id="vendor_special_import_name_0" placeholder="Name"
                name="vendor_special_import_name[]" readonly></td>
        <td>
            <select class="form-select" data-control="select2" id="charge_special_import_id_0"
                name="charge_special_import_id[]" data-key="0">
                <option>Select</option>
                @foreach ($charge as $rows)
        ...
        <td> <select class="form-select" data-control="select2" id="currency_special_import_id_0"
                name="currency_special_import_id[]" data-key="0">
                <option>Select</option>
                @foreach ($currency as $rows)
        ...
        <td><input type="text" class="form-control" name="amount_special_import[]" placeholder="Type here..">
        </td>
        <td><input type="text" class="form-control" name="local_amount_special_import[]"
...
@push('js')
    <script>
        ...
        function setVendorSpecialImportName(key) {
            ...
            setVendorSpecialImport(vendorId, vendorName, vendorCode, '{{ $joborder->loading_plan_id }}');
            @foreach ($bl as $r)
                var value = '{{ $r->bl_number }}';
                setChargeBl(vendorId, vendorName, vendorCode, value, 'special-bl', 'all');
            @endforeach
                ...
                success: function(response) {
                    if (response.status) {
                        ...
                        charges.forEach((charge, idx) => {
                            ...
                            setChargeSpecialImport(data, indx, typex);
        ...
        function setChargeSpecialImport(data, index, type) {
            if (type == 'parent') {
                $('#row-special-import-' + (index - 1)).remove();
            }
            ...
            <tr id="row-special-import-${index}">
                ...
                <div ${type == 'child' ? 'style="display:none"' : ''}>
                    <select class="form-select vendor-select" onchange="setVendorSpecialImportName(${index})"
                ...
                <td>
                <div ${type == 'child' ? 'style="display:none"' : ''}>
                <input type="text" class="form-control" id="vendor_special_import_name_${index}" value='${vendor_name}' placeholder="Name" name="vendor_special_import_name[]" readonly></td>
                </div>
                <td>
                    <select class="form-select" data-control="select2" id="charge_special_import_id_${index}" name="charge_special_import_id[]" data-key="${index}">
                        <option>Select</option>
                        @foreach ($charge as $rows)
                ...
                <td>
                    <select class="form-select" data-control="select2" id="currency_special_import_id_${index}" name="currency_special_import_id[]" data-key="${index}">
                        <option>Select</option>
                        @foreach ($currency as $rows)
