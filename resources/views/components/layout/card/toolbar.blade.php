...
{{-- <div class="card-toolbar flex-row-fluid justify-content-start gap-2">
    ...
    <div class="d-flex align-items-center gap-2">
        ...
        <button type="button" class="btn btn-sm custom-btn custom-btn-primary" data-bs-toggle="modal"
            data-bs-target="#custom_link_modal" id="custom-link-button">
            @if (!empty($customLink['icon']))
            <i class="{{ $customLink['icon'] }}"></i>
            @endif
            {{ $customLink['text'] }}
    ...
    <button type="button" class="btn btn-sm custom-btn custom-btn-outline-primary" data-kt-menu-trigger="click"
...
<div class="card-title">
    @if ($withFilter)
    <a href="javascript:void(0)" class="text-hover-primary ms-2 clear-filter d-none clear-filter-popup"
        ...
        <ul class="filter-list">
                    ...
                    <div class="dropdown dropdown-action" data-bs-toggle="tooltip" data-bs-placement="bottom"
                        ...
                        <div class="dropdown-menu dropdown-menu-end">
                            <ul class="d-block">
                                ...
                                @if (!empty($exportExcelLink))
                                    <li>
                                        <a class="d-flex align-items-center download-item" href="{{ $exportExcelLink }}">
                                ...
                                @if (!empty($exportPdfLink))
                                <li>
                                    <a class="d-flex align-items-center download-item" href="{{ $exportPdfLink }}">
            ...
            @if (!empty($customLink))
                <li>
                    ...
                    @elseif (isset($customLink['is_modal']))
                    <button type="button" class="btn btn-import" data-bs-toggle="modal"
                        data-bs-target="#custom_link_modal" id="custom-link-button">
                        @if (!empty($customLink['icon']))
                        <i class="{{ $customLink['icon'] }}"></i>
                        @endif
                    ...
                    @else
                    <button type="button" class="btn btn-import" id="custom-link-button">
                        @if (!empty($customLink['icon']))
                        <i class="{{ $customLink['icon'] }}"></i>
                        @endif
                        {{ $customLink['text'] }}
