...
@section('title', 'Air Export Costing')
    ...
    <x:layout.card.wrapper>
        <x:layout.card.header>
            ...
            <x:layout.card.toolbar
                exportCsvLink="{{ route('finance.costing.dubai-business.air-export.export.csv') }}"
                withFilter="true"
            />
        </x:layout.card.header>
        <x:layout.card.body>
...
@push('js')
    @component('components.layout.table.datatable', [
        'id' => 'cross-air_table',
        'url' => route('finance.costing.dubai-business.air-export.list'), // Sesuaikan route sesuai kebutuhan Anda
        'dynamicParam' => true,
        'columns' => [
            [
                "data" => "DT_RowIndex",
    ...
    <script>
        $(document).ready(function () {
            ['mawb_number_filter', 'carrier_filter'].forEach(item => {
                ...
                let url = "{{ route('api.finance.costing.dubai_business.air_export.data_filter', ['column' => ':column']) }}";
