...
@endpush
@section('body')
<x:layout.breadcrumb.wrapper module="Costing" pageName="Trucking">
...
<x:layout.card.wrapper>
    ...
    <x:layout.card.body>
     <x:form.wrapper id="costing-form" action="{{ $data['action'] }}" method="{{ $data['method'] }}">
        ...
        <input type="hidden" name="type_costing" value="TRUCKING">
        ...
        <div class="col-md-12">
            <div class="row">
                ...
                <div class='col-md-4'>
                    <x:form.input label="CTD Number" placeholder="CTD Number" name="loading_plan_number" :model="$joborder" disabled="disabled" />
                </div>
                <div class='col-md-4'>
                    <div class='mb-10'>
            ...
            <ul class="nav nav-tabs costing-tabs default-costing-tabs" id="myTab" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link active" id="import-tab" data-bs-toggle="tab" data-bs-target="#import" type="button" role="tab">TRUCKING COSTING</button>
                </li>

            </ul>
            <div class="tab-content p-5 bg-white border border-top-0">
                <div class="tab-pane fade show active" id="import" role="tabpanel">
                    <x-costing.special-other :costing="$costing" type="trucking" :vendorLine="$vendor_all" :charge="$charge" :currency="$currency" :joborder='$joborder'/>
...
function setChargeOther(vendorId,vendorName,vendorCode, number, vendorType, type, typeIndex = '') {
      ...
      $(`.${typeIndex}`).each(function () {
            ...
            //ajax disini
           $.ajax({
            url: `/finance/costing/dubai-business/trucking/contract/${vendorId}/${number}/${type}`, // Replace with your actual route
