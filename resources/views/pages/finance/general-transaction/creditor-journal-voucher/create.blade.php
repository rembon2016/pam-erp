...
@section('body')
    <x:layout.breadcrumb.wrapper module="General Transaction" pageName="{!! $data['page'] !!}">
        ...
        <x:layout.breadcrumb.item pageName="Creditor Journal Voucher"
            href="{{ route('finance.general-transaction.creditor-journal-voucher.index') }}" />
    ...
    <x:form.wrapper action="{{ $data['action'] }}" method="{{ $data['method'] }}">
        <div class="row">
            ...
            <div class='col-md-3'>
                <x:form.select2 label="Invoice/Credit Note" name="invoice_cn" placeholder="Select Invoice/Credit Note" required="true">
                    @foreach (['icn a', 'icn b'] as $row)
                        <option value="{{ $row }}">
                            {{ $row }}
                        </option>
                    @endforeach
            ...
            <div class='col-md-3'>
                <x:form.select2 label="Currency" name="Currency" placeholder="Select Currency" required="true">
                    @foreach (['currency a', 'currency b'] as $row)
                        <option value="{{ $row }}">
                            {{ $row }}
                        </option>
                    @endforeach
